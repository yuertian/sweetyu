...
//        System.out.println(fileList);
    ...
    private static String convertLine(File f) throws IOException {
        ...
        // 4. 把这三个部分拼成一行文本
        //    \3 起到分割三个部分的作用，表示文本结束（Ascll 值为 3 的字符）
        //    在一个 html 这样的文本文件中是不会存在 \3 这种不可见字符的
        //    类似的，使用\1 \2 \3 \4 \5....来分割，效果也一样
    ...
    private static String convertTitle(File f) {
        ...
        // 全路径：E:\jdk1.8.0_73\docs\api\org\xml\sax\EntityResolver.html
    ...
    private static void enumFile(String inputPath, ArrayList<File> fileList) {
        // 递归的把 inputPath 对应的所有目录和文件都遍历一遍
        File root = new File(inputPath);
        // listFiles 相当于 Linux 上的 List 命令
        // 就把当前目录下的所有文件和目录都罗列出来，当前目录就是 root 对象
